import { View, Text, StyleSheet, TextInput, ActivityIndicator, TouchableOpacity,Pressable, Keyboard, SafeAreaView, Share  } from 'react-native'
...
export default function Dolar() {
    ...
    const [dolarResponse, setDolarResponse] = useState()
    const [loading, setLoading] = useState(true)
  
    const [inputReal, setInputReal] = useState()
    const [inputDolar, setInputDolar] = useState()
    const [resultadoFinal,setResultadoFinal] = useState()
    const [resultadoFinalDolar,setResultadoFinalDolar] = useState()
    ...
    function resultado(){
        if(inputReal == null){
         alert('Por favor, digite um valor numérico.')
            ...
            let valorInput = moedaDolar(Number(inputReal))
            setResultadoFinal(
    ...
    function resultadoDolar(){
        if(inputDolar == null){
            alert('Por favor, digite um valor numérico.')
           }else{
        let multiplicacao = moedaDolar(Number(inputDolar*dolarResponse.ask)) 
        let valorInput = moedaReal(Number(inputDolar))
        setResultadoFinalDolar(
  ...
  useEffect(()=>{
    async function realApi() {
        try{
            ...
            setRealResponse(response.data.USD);
            const responseDolar = await api.get('BRL-USD');
            setDolarResponse(responseDolar.data.BRL)
            setLoading(false)
        } catch {}
    }
    realApi()
       ...
       <Pressable onPress={Keyboard.dismiss} style={{height:700}}>
          <View style={styles.Container}>
            <View style={{flexDirection:'row'}} >
                <Text style={{ textAlign:'center', fontSize:22, marginTop:10, marginLeft:20,color:'#fff', fontWeight:'600'}}>
                US$ 1,00 Dolar é igual a
                    </Text>
                    <Text style={{ textAlign:'center', fontSize:22, marginTop:10, color:'#000', fontWeight:'bold' }}>
                    {` ${moedaReal(Number(realResponse.ask))}`}
                    </Text>
            </View>
            <Text style={{marginLeft:23, marginTop:5, fontSize:12, color:'#000'}}>{realResponse.create_date}</Text>
            <View style={styles.ContainerBRL}>
                <Text style={{marginLeft:5, marginRight:10, fontSize:22,color:'#fff',fontWeight:'600'}}>US$</Text>
          ...
          <View style={{flexDirection:'row'}} >
                <Text style={{ textAlign:'center', fontSize:22, marginTop:10, marginLeft:25,color:'#fff',fontWeight:'600'}}>
                    ...
                    <Text style={{ textAlign:'center', fontSize:22, marginTop:10, color:'#000', fontWeight:'bold' }}>
                    {` ${moedaDolar(Number(dolarResponse.ask))}`}
                    </Text>
            </View>
            ...
            <View style={styles.ContainerBRL}>
                <Text style={{marginLeft:20, marginRight:10, fontSize:22,color:'#fff',fontWeight:'600'}}>R$</Text>
                    ...
                    placeholder="2,00"
                    value={inputDolar}
                    onChangeText={(valor) => valorDolar(valor) } />
            </View>
            <View style={styles.button}>
                      <TouchableOpacity onPress={resultadoDolar} style={{ borderRadius:10 , height:40, justifyContent:'center', marginHorizontal:40}} >
                          <Text style={{textAlign:'center', fontSize:22, color:'#fff', fontWeight:'500'}}>Converter</Text>
                      </TouchableOpacity>
            </View>
