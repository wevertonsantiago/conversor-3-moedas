...
useEffect(()=>{
  async function realApi() {
      try{
          ...
          setRealResponse(response.data.EUR);
          const responseDolar = await api.get('BRL-EUR');
     ...
     <Pressable onPress={Keyboard.dismiss}style={{height:700}}>
        <View style={styles.Container}>
          <View style={{flexDirection:'row'}} >
              <Text style={{ textAlign:'center', fontSize:22, marginTop:10, marginLeft:30,color:'#fff', fontWeight:'600'}}>
              € 1,00 Euro é igual a
                  </Text>
          ...
          <Text style={{marginLeft:32, marginTop:5, fontSize:12, color:'#000'}}>{realResponse.create_date}</Text>
        ...
        <View style={{flexDirection:'row'}} >
          ...
          <View style={styles.button}>
                    <TouchableOpacity onPress={resultadoDolar} >
                        <Text style={{textAlign:'center', fontSize:22, color:'#fff', fontWeight:'500'}}>Converter</Text>
                    </TouchableOpacity>
          </View>
